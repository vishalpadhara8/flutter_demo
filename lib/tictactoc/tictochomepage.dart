    ...
    setState(() {
      ...
      if (winner == -1) {
         if (buttonList.every((p) => p.text != "")) {
           showDialog(
             context: context,
               builder: (_) => new CustomDialog("Game Tied", "Press the reset button to start again.", resetGametic));
         } else {
  ...
  int CheckWinner() {
    ...
    if (winner != -1) {
        if(winner == 1){
          showDialog(
            context: context,
            builder: (_) => new CustomDialog("Player 1 Won", "Press the reset button to start again.", resetGametic)
          );
        } else {
          showDialog(
              context: context,
              builder: (_) => new CustomDialog("Player 2 Won", "Press the reset button to start again.", resetGametic)
          );
        }
